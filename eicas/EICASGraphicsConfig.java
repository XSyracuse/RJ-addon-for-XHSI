...
public class EICASGraphicsConfig extends GraphicsConfig implements ComponentListener {
    ...
    //2018
    public int crj_fuel_primary_x[] = new int[3];
    public int crj_fuel_primary_y[] = new int[3];
    ...
    public void update_config(Graphics2D g2, boolean power, int instrument_style, int nb_engines, int detents ) {
        ...
        if (this.resized
            ...
            //CRJ
            gears_w = panel_rect.width*39/100;
            gears_x0 = panel_rect.x + panel_rect.width - alerts_w;
            gear_h = line_height_xs*25/10;
            gear_y0 = panel_rect.y + eicas_size*60/100;
            
            
            crj_fuel_primary_x[0] = alerts_x0 + alerts_w/2;
            crj_fuel_primary_x[1] = alerts_x0 + alerts_w/2 - fuel_r*5/3;
            crj_fuel_primary_x[2] = alerts_x0 + alerts_w/2 + fuel_r*5/3;
            
            crj_fuel_primary_y[1] = panel_rect.y + panel_rect.height - fuel_r*3/2;
            crj_fuel_primary_y[2] = crj_fuel_primary_y[1];
            crj_fuel_primary_y[0] = crj_fuel_primary_y[1];

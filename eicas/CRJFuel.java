...
public class CRJFuel extends EICASSubcomponent {
    ...
    public CRJFuel(ModelFactory model_factory, EICASGraphicsConfig hsi_gc, Component parent_component) {
...
//            String units_str = this.preferences.get_preference(XHSIPreferences.PREF_FUEL_UNITS);
            ...
            if ( primaries ) {
                String FuelLabel = "FUEL QTY (" + units_str + ")";
                g2.drawString(FuelLabel, eicas_gc.fuel_primary_x[0] - eicas_gc.get_text_width(g2, eicas_gc.font_m, FuelLabel)/2, eicas_gc.fuel_primary_y[0] + eicas_gc.fuel_r - 2);
                ...
                int fuel_y = eicas_gc.crj_fuel_primary_y[0];
                int total_y = fuel_y + 4 * eicas_gc.line_height_xl/2 - 2;
                String total = "" + Math.round( this.aircraft.get_total_fuel() * this.aircraft.fuel_multiplier() );
                String TotalLabel = "TOTAL FUEL " + total;
                g2.drawString(TotalLabel, eicas_gc.fuel_primary_x[0] - eicas_gc.get_text_width(g2, 
                              eicas_gc.font_xl, TotalLabel)/2, total_y);
            } else {
...
//range = 2000.0f;
        ...
        int fuel_x = eicas_gc.crj_fuel_primary_x[tank];
        int fuel_y = eicas_gc.crj_fuel_primary_y[tank];
        //System.out.println(tank+" "+fuel_y);
        
        /*
        if(tank==0) {
          fuel_y = prims ? eicas_gc.fuel_primary_y[1] : eicas_gc.fuel_compact_y[1];
        }
        */
